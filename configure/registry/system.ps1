...
$systemCurrentStep++; $statusMessage = "Setting Global Codepage..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
...
$systemCurrentStep++; $statusMessage = "Setting Registry: Console Startup..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
...
$systemCurrentStep++; $statusMessage = "Setting Registry: PowerShell Execution Policy..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
...
$systemCurrentStep++; $statusMessage = "Setting Registry: Explorer Advanced Settings..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
...
$systemCurrentStep++; $statusMessage = "Setting Registry: Taskbar Developer Settings..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
Set-RegistryDword -Path "HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Advanced\\TaskbarDeveloperSettings" -Name "TaskbarEndTask" -Value 1

$systemCurrentStep++; $statusMessage = "Setting Registry: Explorer Policies..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
...
$systemCurrentStep++; $statusMessage = "Setting Registry: Search Settings..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
...
$systemCurrentStep++; $statusMessage = "Setting Registry: AppModelUnlock..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
...
$systemCurrentStep++; $statusMessage = "Setting Registry: LogonUI Background..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
...
$systemCurrentStep++; $statusMessage = "Setting Registry: Sudo Config..."; Write-Progress -Activity "System Registry Configuration" -Status $statusMessage -PercentComplete ([Math]::Min(100, (($systemCurrentStep / $systemTotalSteps) * 100))) -Id $progressIdSystem
I $statusMessage
